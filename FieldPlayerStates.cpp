...
//TODO add defensive play and player controls
...
bool GlobalPlayerState::OnMessage(FieldPlayer* player, const Telegram& telegram)
{
	...
	case Msg_DefaultState:
		{
			//change the state
			player->GetFSM()->ChangeState(DefaultState::Instance());
...
void ChaseBall::Execute(FieldPlayer* player)                                     
{
	...
	//to his home region and DefaultState for another opportunity
	player->GetFSM()->ChangeState(ReturnToHomeRegion::Instance());
}
...
void ReturnToHomeRegion::Execute(FieldPlayer* player)
{
	...
	//if game is on and close enough to home, change state to DefaultState and set the 
...
void DefaultState::Enter(FieldPlayer* player)
{
#ifdef PLAYER_STATE_INFO_ON
	debug_con << "Player " << player->ID() << " enters DefaultState state" << "";
#endif
...
void DefaultState::Execute(FieldPlayer* player)
{    
	...
	if (player->Pitch()->GameOn())
	{
		...
		// if the player is in control of the ball and is in the final third
		if (player->isControllingPlayer() && player->InHotRegion())
		{
			player->GetFSM()->ChangeState(ChaseBall::Instance()); // chase the ball in the hope of passing/shooting
		}
	} // ends if for if game on
}

void DefaultState::Exit(FieldPlayer* player){}
...
{ 
	...
	if (player->isThreatened()  &&
		...
		BallTarget,
		power,
		Prm.MinPassDist) && player->Team()->IsControlledByComputer() == false)
	{     
		...
		//the player should go back to the DefaultState at his current position unless instructed
		//otherwise  
		player->GetFSM()->ChangeState(DefaultState::Instance());
...
void ReceiveBall::Execute(FieldPlayer* player)
{
	...
	//if the player has 'arrived' at the steering target he should DefaultState and
