...
void SoccerTeam::Update()
{
  ...
  for (it; it != m_Players.end(); ++it)
  {
	...
	void SoccerTeam::InitialiseControls()
	{
	CreatePlayers();
	if (m_bControlledByComputer == false)
	{
		std::vector<PlayerBase*>::iterator it = m_Players.begin();
		for (it; it != m_Players.end(); ++it)
		{
...
void SoccerTeam::CreatePlayers()
{
  ...
  {
    //goalkeeper
    m_Players.push_back(new GoalKeeper(this, // http://www.cplusplus.com/reference/vector/vector/push_back/
                               1,
                               TendGoal::Instance(),
                               Vector2D(0,1),
                               Vector2D(0.0, 0.0),
...
//---------------------- UpdateTargetsOfDefaultStateingPlayers ------------------------
//
//  
void SoccerTeam::UpdateTargetsOfDefaultStateingPlayers()const
{
  std::vector<PlayerBase*>::const_iterator it = m_Players.begin();
  ...
  {  
    if ( (*it)->Role() != PlayerBase::goal_keeper )
    {
      ...
      if ( plyr->GetFSM()->isInState(*DefaultState::Instance()) ||
