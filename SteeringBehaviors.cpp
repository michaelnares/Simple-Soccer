...
//  ball
//------------------------------------------------------------------------
Vector2D SteeringBehaviors::Pursuit(MovingEntity* entity)
{
  Vector2D ToEntity = entity->Pos() - m_pPlayer->Pos();
  ...
  if (entity->Speed() != 0.0)
  {
    LookAheadTime = ToEntity.Length() / entity->Speed();
  }

  //calculate where the ball will be at this time in the future
  m_vTarget = entity->FuturePosition(LookAheadTime);
