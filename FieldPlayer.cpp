...
void FieldPlayer::CalculateClosestOpposingPlayer() 
{
Vector2D curPos = Pos(); //i.e. this player's position
...
//check against all opponents to make sure non are within this
  ...
  {
		double dist = (Vec2DDistanceSq(Pos(), (*curOpp)->Pos());
    {
		if (dist < ClosestSoFar) 
		{
			ClosestSoFar = dist;
		}
      m_pClosestOpponent = *curOpp;
    }
  } // ends for loop
} // ends method
